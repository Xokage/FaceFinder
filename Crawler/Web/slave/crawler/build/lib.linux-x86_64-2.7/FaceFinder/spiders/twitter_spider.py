...
class TwitterSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        try:
            ...
            if exception.errno != errno.EEXIST:
                raise
        try:
            os.makedirs(os.path.join("gallery",self.image_dir))
        except OSError as exception:
            if exception.errno != errno.EEXIST:
                raise
        try:
            ...
            if exception.errno != errno.EEXIST:
                raise
        try:
            os.makedirs(os.path.join('downloads',self.downloads_dir))
        except OSError as exception:
            if exception.errno != errno.EEXIST:
                raise
        ...
        #Add images to br
        files = []
        directories = [os.path.join('gallery',self.image_dir)]                    
        logging.debug("Loading images from %s." % os.path.join('gallery',self.image_dir))
    ...
    def get_image(self, url):
        ...
        if (not os.path.isfile(os.path.join('downloads',self.downloads_dir) + filename + '.jpg')):
            while attempts < 3:
                try:
                    response = urllib2.urlopen(url, timeout = 5)
                    content = response.read()
                    f = open(os.path.join('downloads',self.downloads_dir) + filename + '.jpg', 'w' )
                    f.write( content )
                    f.close()
                    break
        ...
        return os.path.join('downloads',self.downloads_dir) + filename + '.jpg'
